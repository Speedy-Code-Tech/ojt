...
$tables = $conn->query("SELECT * FROM application_table WHERE after_ojt_status = 'PENDING' AND after_ojt_steps = 3 AND pi_course = '$prog'");
...
<body>
    ...
    <!-- Main Content -->
    <div class="dashboard">
        <h4>2ND STEP PENDING FOR APPROVAL APPLICATIONS FOR <?= strtoupper($data['department'])  ?> </h4>
        <hr>
        ...
        $(document).ready(function() {
            ...
            $('.approve-btn').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = `updates.php?id=${applicationId}&status=${status}`;
                    }
                });
            });
            ...
            $('.decline-btn').on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Redirect to update.php with the application ID
                        window.location.href = `updates.php?id=${applicationId}`;
                    }
                });
            });
        });
