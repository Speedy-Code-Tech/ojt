...
<body>
    ...
    <div class="dashboard">
        ...
        <a href="add.php" class="btn btn-success" style="position: absolute; right: 30px; top: 20px;"><i class="fa-solid fa-plus"></i> Add New Admin</a>
        <hr>
        <table class="table table-hover table-striped" id="announcement">
            ...
            <?php
                ...
                while ($row = $result21->fetch_assoc()) {
                    ...
                    $btnclass1 = $status=='Deactivated'?'d-block':'d-none';
                    echo "<tr>
                        <td>{$no}</td>
                        <td>{$row['email']}</td>
                        ...
                        <td class='d-flex gap-2'>
                            ...
                            <button class='btn btn-danger btn-sm delete-btn {$btnclass}' data-id='{$row['user_id']}'>DeActivate</button>
                            <button class='btn btn-primary btn-sm activate-btn {$btnclass1}' data-id='{$row['user_id']}'>Activate</button>
                        </td>
                    </tr>";
                    $no++;
                }
    ...
    $('.delete-btn').on('click', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = `delete.php?id=${userId}&status=Deactivated`;
            }
        });
    });
    ...
    $('.activate-btn').on('click', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = `delete.php?id=${userId}&status=Activated`;
            }
        });
    });
});
