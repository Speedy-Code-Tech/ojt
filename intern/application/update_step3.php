...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $step3_docs_link = mysqli_real_escape_string($conn, $_POST['step3_docs_link']);

    // Validate the G-Link (optional)
    if (filter_var($step3_docs_link, FILTER_VALIDATE_URL)) {
        // Update the database with the new G-Link
        $update_query = "UPDATE application_table SET step3_docs_link = '$step3_docs_link' WHERE user_id = '$user_id'";
